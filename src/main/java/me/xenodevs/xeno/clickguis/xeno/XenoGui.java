package me.xenodevs.xeno.clickguis.xeno;
...
public class XenoGui extends ClickGuiBase {

    public static ArrayList<Frame> frames;
    public static int color = clickGuiColor;
    ...
    public XenoGui() {
        ...
        int x = (DisplayUtils.getDisplayWidth() / 2) - ((Category.values().length * (frameWidth + 6)) / 2);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for(Frame frame : frames) {
            ...
            for(Component comp : frame.getComponents()) {
                ...
                if(comp instanceof Button && mouseOver(frame.getX() - 2, frame.getY() + 3 + frameBarHeight, frame.getX() + frameWidth + 2, frame.getY() + 3 + frameBarHeight + (maxLength), mouseX, mouseY)) {
                    if(((Button) comp).isMouseOnButton(mouseX, mouseY)) {
                        descText = ((Button) comp).mod.description;
                        renderDesc = ClickGuiModule.desc.isEnabled() && frame.isOpen() && frame.animateXenoTheme.getValue() == frame.animateXenoTheme.getMax();
                    }
                }
            }

            if(limitFrameXandYToDisplaySides) {
                if(frame.getX() - 2 < 0)
                    frame.setX(2);
        ...
        if(renderDesc) {
            Xeno.blurManager.blur(mouseX + 5, mouseY - 15, TextUtil.getStringWidth(descText) + 5, 15, 3);
            TextUtil.drawStringWithShadow(descText, mouseX + 7, mouseY - 13, -1);
        }
    ...
    protected void mouseClicked(final int mouseX, final int mouseY, final int mouseButton) throws IOException {
        for(Frame frame : frames) {
            ...
            if(frame.isOpen() && frame.animateXenoTheme.getValue() > 0 &&
                    mouseOver(frame.getX() - 2, frame.getY() + 4 + frameBarHeight, frame.getX() + frameWidth + 2, frame.getY() + 4 + frameBarHeight + (maxLength), mouseX, mouseY)
    ...
    public void checkMouseWheel(int mouseX, int mouseY) {
        ...
        if (dWheel < 0) {
            for(Frame f : frames) {
                if (f.isOpen() && mouseOver(f.getX() - 2, f.getY() + frameBarHeight + 3, f.getX() + frameWidth + 2, f.getY() + frameBarHeight + 3 + (maxLength), mouseX, mouseY)) {
                    for (Component c : f.components) {
                        if (c instanceof Button) {
                            Button button = (Button) c;
                            if(!(((Button)f.getComponents().get(0)).offset + 0.5 > frameBarHeight + 2.5))
                                button.offset += buttonBarHeight / 2;
                        }
                    }
                }
            }
        ...
        for(Frame f : frames) {
            ...
            if (!last.open) {
                if (last.offset < frameBarHeight + 3) {
                    last.offset = frameBarHeight + 3 + buttonBarHeight;
                    f.refresh();
                    return;
                }
            } else {
                ...
                if (lastSetting.offset < frameBarHeight + 3) {
                    lastSetting.offset = frameBarHeight + 3 + buttonBarHeight;
                    f.refresh();
                    return;
                }
            }
